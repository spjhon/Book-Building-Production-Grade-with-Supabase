...
export async function GET(request: NextRequest) {
    ...
    if (!imagePath){
        return NextResponse.redirect(buildUrl(`/auth/error?type=No hay imagen que procesar`, tenant, request), { status: 303 });
    }
  ...
  .download(imagePath, {
    ...
    if (errorTransformingTheImage){
        return NextResponse.redirect(buildUrl(`/auth/error?type=${errorTransformingTheImage.message ?? "error al tranformar la imagen"}`, tenant, request), { status: 303 });
    }
