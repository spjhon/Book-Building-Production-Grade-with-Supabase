...
export function buildUrl(applicationPath: string, tenant: string, request: NextRequest) {
  ...
  // Retornamos una URL absoluta: ej: http://acme.miapp.com:3000/tickets
...
Como tu aplicación ahora depende del subdominio (ej: acme.miapp.com), necesitas extraer ese texto constantemente.

    En lugar de escribir el código de split(":") en cada archivo, llamas a esta función.

    Al obtener el hostname, puedes consultar tu TENANT_MAP para saber si el cliente es "Acme", "Globex", etc.

2. Manejo del Puerto (Entorno de Desarrollo)

En producción, las URLs no suelen mostrar el puerto (https://miapp.com), pero en tu computadora usas localhost:3000.
