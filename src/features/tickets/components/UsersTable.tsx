  ...
  useEffect(() => {
    async function loadTenantData() {
      ...
      try {
        ...
        const { data: usersFetched, error: usersFetchedError } =
          await supabase.rpc("get_service_users_with_tenant", {
            target_tenant_id: tenantData.id,
          });

        if (usersFetchedError) {
          router.push(
            "/error?type=No fue posible obtener informacion" +
              usersFetchedError.message,
          );
          return;
        }
        ...
        const { data: sessionInfo, error: sessionInforError } =
