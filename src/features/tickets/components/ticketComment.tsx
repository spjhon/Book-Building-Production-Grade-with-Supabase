...
const TicketComments = ({
  ...
  const t = useTranslations("TicketComments");

  const supabaseBrowser = createSupabaseBrowserClient();
  const textareaRef = useRef<HTMLTextAreaElement>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [commentsState, setComments] = useState<TicketComment[]>(
    comments || [],
  );
    ...
    const subscription = supabaseBrowser
      ...
      .on(
        ...
        async (payload) => {
          ...
          const { data: attachments, error: errorFetchngNewComment } =
          ...
          if (errorFetchngNewComment) {
            alert(
              "error trallendo los adjuntos, lo siento: " +
                errorFetchngNewComment.message,
            );
          }
    ...
    try {
      ...
      if (fileList && fileList.length > 0) {
        const uploadPromises = Array.from(fileList).map(async (file) => {
          ...
          const { data, error: errorUploadingFiles } =
      ...
      const { data: commentData, error: errorCreatingComment } =
        await supabaseBrowser
          .from("comments")
          .insert({
            ticket_id,
      ...
      if (uploadedFiles.length > 0) {
        ...
        if (attachError) {
          throw {
            contexto:
              "Error al crear la relacion en la tabla comment_attachments", // Tu mensaje personalizado
      ...
      console.log(err);
      console.log(
        "esta es la variable commentIDforDeleteInCaseOfError: " +
          commentIDforDeleteInCaseOfError,
      );
      ...
      if (commentIDforDeleteInCaseOfError) {
        const { data: dataDeletingComment, error: errorDeletingComment } =
          await supabaseBrowser
            .from("comments")
            .delete()
        ...
        if (dataDeletingComment)
          console.log("Se borro el comentario correctamente");
        if (errorDeletingComment) {
          console.log(
            errorDeletingComment.message + "Error borrando el comentario",
          );
        }
      }
      ...
      if (uploadedFiles.length > 0) {
        ...
        const {
          data: deletingAttachmentsData,
          error: errorDeletingAttachments,
        } = await supabaseBrowser.storage
          .from("comments-attachments")
          .remove(pathsToDelete);

        if (deletingAttachmentsData)
          console.log("se borraron los attachments correctamente");
        if (errorDeletingAttachments)
          console.log(
            "error en el borrado de attachments" + errorDeletingAttachments,
          );
      }

      alert(t("alert_error_saving_comment"));
    } finally {
    ...
    <footer className="mt-8 space-y-10">
      ...
      <form onSubmit={handleSubmit} className="relative group">
        <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500 transition-all shadow-sm">
          <textarea
            ref={textareaRef}
            placeholder={t("placeholder_write_comment")}
                    ...
                    <div className="flex flex-wrap gap-3">
                      {comment.comment_attachments.map((attachment) => (
                        <div key={attachment.id} className="group relative">
                          <button
                            onClick={() =>
                              onClickHandlerButtonDownload(attachment.file_path)
                            }
                          ...
                          {attachment.file_path.endsWith(".png") && (
                            <div className="mt-2 rounded-lg overflow-hidden border border-slate-200">
                              <Image
                                alt="thumbnail"
                                src={urlPath(
                                  `/cdn/api?image=${attachment.file_path}`,
                                  tenantName,
                                )}
                                width={120}
                                height={120}
